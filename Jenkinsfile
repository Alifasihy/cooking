pipeline {
  ...
  stages {
	...
	sh 'docker network inspect -f "{{range .Containers}}{{println .Name}}{{end}}" cooking|xargs -r docker rm -fv'
        sh 'docker-compose up --detach && sleep 100'
	sh 'docker run --network cooking -d -p 3000:3000 registry:5000/cookingmatch:latest'
      }
    }
  }
}
